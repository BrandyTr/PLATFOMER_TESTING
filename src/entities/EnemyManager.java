...
public class EnemyManager {
    ...
    private void loadEnemyImgs() {
        ...
        BufferedImage temp = LoadSave.GetSpriteAtlas(LoadSave.FOX_ENEMY);
        for(int j = 0; j < foxArr.length; j++)
            for(int i = 0; i < foxArr[j].length; i++)
                foxArr[j][i] = temp.getSubimage(i * FOX_WIDTH_DEFAULT, j * FOX_HEIGHT_DEFAULT, FOX_WIDTH_DEFAULT, FOX_HEIGHT_DEFAULT);
